"""DDSurfer surface prediction pipeline (left hemisphere defaults).

This module orchestrates volumetric feature loading and surface prediction for
the white-matter (WM) and pial cortical surfaces using the cascaded DDSurfer
dual-stream TANet. The numerical model invocation mirrors the historical
implementation while improving structure, logging, and configurability for
open-source release.
"""
...
class PredictionConfig:
    """Container for run-time configuration of the prediction pipeline."""
    ...
    @property
    def slice_offset(self) -> np.ndarray:
        if self.hemisphere == "right":
            return np.array([64.0, 0.0, 0.0], dtype=np.float32)
        return np.zeros(3, dtype=np.float32)

    @property
    def translation_vector(self) -> np.ndarray:
        # Historical translation used in legacy script.
...
class SurfacePredictor:
    """Wraps the DDSurfer TANet model for WM and pial surface prediction."""
    ...
    def __init__(self, cfg: PredictionConfig) -> None:
        ...
        self.vert_init = mesh_init.vertices
    ...
    def predict(self, subject_dir: Path, output_dir: Path) -> float:
        ...
        if self.cfg.predict_mode == "all" and verts_pial_np is not None:
            transformed_pial = (verts_pial_np + slice_offset) - translation
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Predict DDSurfer cortical surfaces.")
